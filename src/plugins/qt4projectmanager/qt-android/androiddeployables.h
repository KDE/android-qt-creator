...
#ifndef ANDROIDDEPLOYABLES_H
#define ANDROIDDEPLOYABLES_H
...
class AndroidDeployables : public QAbstractListModel
{
    Q_OBJECT
public:
    AndroidDeployables(const ProjectExplorer::BuildStep *buildStep);
    ~AndroidDeployables();
    void setUnmodified();
    bool isModified() const;
    int deployableCount() const;
    AndroidDeployable deployableAt(int i) const;
    ...
    AndroidDeployableListModel *modelAt(int i) const { return m_listModels.at(i); }
    const ProjectExplorer::BuildStep *buildStep() const { return m_buildStep; }

private:
    typedef QHash<QString, AndroidDeployableListModel::ProFileUpdateSetting> UpdateSettingsMap;
    ...
    QList<AndroidDeployableListModel *> m_listModels;
