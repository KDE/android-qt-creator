...
AndroidUsedPortsGatherer::AndroidUsedPortsGatherer(QObject *parent) :
    QObject(parent), m_running(false)
{
}

AndroidUsedPortsGatherer::~AndroidUsedPortsGatherer() {}

void AndroidUsedPortsGatherer::start(const Core::SshConnection::Ptr &connection,
    const AndroidPortList &portList)
{
    if (m_running)
    ...
    const QString command = AndroidGlobal::remoteSudo()
...
void AndroidUsedPortsGatherer::stop()
{
    if (!m_running)
        return;
    m_running = false;
...
int AndroidUsedPortsGatherer::getNextFreePort(AndroidPortList *freePorts) const
{
...
void AndroidUsedPortsGatherer::setupUsedPorts()
{
...
void AndroidUsedPortsGatherer::handleConnectionError()
{
    if (!m_running)
        return;
...
void AndroidUsedPortsGatherer::handleProcessClosed(int exitStatus)
{
    if (!m_running)
        return;
    QString errMsg;
...
void AndroidUsedPortsGatherer::handleRemoteStdOut(const QByteArray &output)
{
    m_remoteStdout += output;
}

void AndroidUsedPortsGatherer::handleRemoteStdErr(const QByteArray &output)
{
