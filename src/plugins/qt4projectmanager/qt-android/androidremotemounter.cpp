...
AndroidRemoteMounter::AndroidRemoteMounter(QObject *parent)
...
AndroidRemoteMounter::~AndroidRemoteMounter()
{
    killAllUtfsServers();
}

void AndroidRemoteMounter::setConnection(const SshConnection::Ptr &connection)
{
    ASSERT_STATE(Inactive);
    m_connection = connection;
}

void AndroidRemoteMounter::setToolchain(const AndroidToolChain *toolChain)
{
    ASSERT_STATE(Inactive);
...
void AndroidRemoteMounter::addMountSpecification(const AndroidMountSpecification &mountSpec,
    bool mountAsRoot)
{
    ASSERT_STATE(Inactive);
...
bool AndroidRemoteMounter::hasValidMountSpecifications() const
{
    return !m_mountSpecs.isEmpty();
}

void AndroidRemoteMounter::mount(AndroidPortList *freePorts,
    const AndroidUsedPortsGatherer *portsGatherer)
{
    ASSERT_STATE(Inactive);
...
void AndroidRemoteMounter::unmount()
{
    ASSERT_STATE(Inactive);
...
void AndroidRemoteMounter::handleUnmountProcessFinished(int exitStatus)
{
...
void AndroidRemoteMounter::stop()
{
    setState(Inactive);
}

void AndroidRemoteMounter::deployUtfsClient()
{
...
void AndroidRemoteMounter::handleUploaderInitializationFailed(const QString &reason)
{
...
void AndroidRemoteMounter::handleUploaderInitialized()
{
    ASSERT_STATE(QList<State>() << UploaderInitializing << Inactive);
    if (m_state == Inactive)
        return;
...
void AndroidRemoteMounter::handleUploadFinished(Core::SftpJobId jobId,
...
void AndroidRemoteMounter::startUtfsClients()
{
    const QString chmodFuse
        = AndroidGlobal::remoteSudo() + QLatin1String(" chmod a+r+w /dev/fuse");
    ...
    for (int i = 0; i < m_mountSpecs.count(); ++i) {
        ...
        const AndroidMountSpecification &mountSpec = mountInfo.mountSpec;
        const QString mkdir = QString::fromLocal8Bit("%1 mkdir -p %2")
            .arg(AndroidGlobal::remoteSudo(), mountSpec.remoteMountPoint);
        ...
        if (mountInfo.mountAsRoot)
            utfsClient.prepend(AndroidGlobal::remoteSudo() + QLatin1Char(' '));
        QLatin1String seqOp("; ");
        remoteCall += seqOp + AndroidGlobal::remoteSourceProfilesCommand()
...
void AndroidRemoteMounter::handleUtfsClientsStarted()
{
...
void AndroidRemoteMounter::handleUtfsClientsFinished(int exitStatus)
{
...
void AndroidRemoteMounter::startUtfsServers()
{
...
void AndroidRemoteMounter::handleUtfsServerStderr()
{
    if (m_state != Inactive) {
...
void AndroidRemoteMounter::handleUtfsServerError(QProcess::ProcessError)
{
...
void AndroidRemoteMounter::handleUtfsServerFinished(int /* exitCode */,
...
void AndroidRemoteMounter::handleUtfsClientStderr(const QByteArray &output)
{
    if (m_state != Inactive)
        m_utfsClientStderr += output;
}

void AndroidRemoteMounter::handleUmountStderr(const QByteArray &output)
{
    if (m_state != Inactive)
        m_umountStderr += output;
}

QString AndroidRemoteMounter::utfsClientOnDevice() const
{
    return AndroidGlobal::homeDirOnDevice(m_connection->connectionParameters().uname)
        + QLatin1String("/utfs-client");
}

QString AndroidRemoteMounter::utfsServer() const
{
    return m_maddeRoot + QLatin1String("/madlib/utfs-server");
}

void AndroidRemoteMounter::killAllUtfsServers()
{
...
void AndroidRemoteMounter::killUtfsServer(QProcess *proc)
{
...
void AndroidRemoteMounter::handleUtfsServerTimeout()
{
...
void AndroidRemoteMounter::setState(State newState)
{
