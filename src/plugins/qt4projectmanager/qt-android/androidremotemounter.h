...
#ifndef ANDROIDREMOTEMOUNTER_H
#define ANDROIDREMOTEMOUNTER_H
...
class AndroidRemoteMounter : public QObject
{
    Q_OBJECT
public:
    AndroidRemoteMounter(QObject *parent);
    ~AndroidRemoteMounter();

    // Must already be connected.
    void setConnection(const QSharedPointer<Core::SshConnection> &connection);

    void setToolchain(const AndroidToolChain *toolChain);
    void addMountSpecification(const AndroidMountSpecification &mountSpec,
        bool mountAsRoot);
    bool hasValidMountSpecifications() const;
    void resetMountSpecifications() { m_mountSpecs.clear(); }
    void mount(AndroidPortList *freePorts,
...
private:
    ...
    struct MountInfo {
        MountInfo(const AndroidMountSpecification &m, bool root)
            : mountSpec(m), mountAsRoot(root), remotePort(-1) {}
        AndroidMountSpecification mountSpec;
        bool mountAsRoot;
        int remotePort;
    };
    ...
    const AndroidUsedPortsGatherer *m_portsGatherer;
